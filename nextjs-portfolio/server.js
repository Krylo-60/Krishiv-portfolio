...
const ROOT_DIR = path.join(__dirname, ".."); // Point back to original data dir
...
async function loadIdeas() {
  ...
  const state = await readAppState("ideas_v1");
...
async function saveIdeas(ideas) {
  ...
  await writeAppState("ideas_v1", inMemoryIdeas);
  try {
    await fs.mkdir(DATA_DIR, { recursive: true });
...
async function loadYtNexus() {
  ...
  const state = await readAppState("yt_nexus_v1");
  if (state) {
...
async function saveYtNexus(nextPayload) {
  ...
  ytNexusBootstrapped = true;
  await writeAppState("yt_nexus_v1", normalized);
  try {
    await fs.mkdir(DATA_DIR, { recursive: true });
...
async function loadUsageAnalytics() {
  if (!usageBootstrapped) {
    const state = await readAppState("usage_v1");
...
async function saveUsageAnalytics(next) {
  ...
  usageBootstrapped = true;
  await writeAppState("usage_v1", usageAnalytics);
  try {
    await fs.mkdir(DATA_DIR, { recursive: true });
    ...
    if (req.method === "POST" && pathname === "/api/aether/chat") {
      ...
      if (!XAI_API_KEY && !getAvailableGoogleKeys().length) {
        sendJson(res, 503, {
          ...
          hint: "Set XAI_API_KEY, GOOGLE_API_KEY, or GOOGLE_API_KEYS in environment variables"
        });
        return;
      }
      ...
      try {
        ...
        sendJson(res, 502, {
          ...
          message: error instanceof Error ? error.message : "Unknown AI API error"
        });
      }
      return;
    }
...
app.prepare().then(() => {
  server.listen(PORT, () => {
  console.log(`Next.js Custom Server running at http://localhost:${PORT}`);
  });
});
